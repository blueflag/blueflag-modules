...
function Coverage(program) {
    ...
    var nycBin = cwd + '/node_modules/proto-blueflag-test/node_modules/.bin/nyc';
    ...
    var args = [].concat(['--all', '--check-coverage', '--branches=' + minCoverage, '--functions=' + minCoverage, '--lines=' + minCoverage, '--statements=' + minCoverage, '--include=src/**', '--exclude=**/*-test.js', '--reporter=text', '--reporter=lcov', '--extension=.js']).concat(program.extraFlags).concat([blueflagTestBin, 'test']);
